...
Инструмент **детерминированной** проверки: файл контракта `specs/<feature>.yaml` сопоставляется с исходниками Python в `openvair/modules/<feature>/` только через разбор AST (рантайм и тесты не выполняются).
...
**Направление соответствия:** всё, что **заявлено в YAML**, **обязано** быть в коде в том же виде. «Лишнее» в коде, чего нет в спеке, **не** является ошибкой (если только вы сами не ужесточаете процесс ревью).
...
| `lint_repo_specs.py` | Прогон **всех** `specs/*.yaml` |
| `generate_openvair_specs.py` | Перегенерация спеки из кода |
| `specs/*.yaml` | Контракты; поле `feature` должно совпадать с именем папки в `openvair/modules/` |
